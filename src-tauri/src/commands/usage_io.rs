...
/// "kimi", never "all". Local rows carry the authoritative integration tag in
/// `p` (and Cursor proxies many model families, so its `p` must win over the
/// model). Aggregated SSH-device rows store `p = "all"`, so for those the tool
/// is recovered from the model family; a third-party/unknown model falls back
/// to "claude".
