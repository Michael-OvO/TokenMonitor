...
- **饼图明细列表滚动条改为 4px 细条**：模型数超出 108px 列表高度时不再显示系统原生滚动条，改为 4px 透明轨道 + 半透明圆角滑块（悬停加深，含浅色主题变体）。仅使用 `::-webkit-scrollbar` 伪元素：一旦同时设置标准的 `scrollbar-width`，引擎会忽略这些伪元素而回退到自带的 ~11px「thin」滚动条
