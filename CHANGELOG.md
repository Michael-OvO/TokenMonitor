...
- **Claude Code 工作时周期性 CPU 尖峰**：statusline 每次写入事件（活跃会话中每 1–8s 一次），2s 轮询都会对全部四个提供方刷新 rate limits，其中 Cursor 没有任何新鲜度门槛，每次都要 spawn `sqlite3` 读 IDE token 再请求 Cursor API（实测每个事件约 110–170ms），Kimi 每 5 分钟对已过期登录发一次注定失败的网络请求——即使这两个标签已在设置里关闭。现在后台刷新与 `get_rate_limits("all")` 只探测已启用的集成（`RateLimitSelection::enabled`，`+` 连接的作用域也按其列出的提供方解析），Cursor 与其他提供方一样有 300s 的最小重取间隔；`statusline-poll` 与 `rate-limits` 各多一条 `[PROFILE]` 日志，便于从 `backend.log` 看到每个事件的耗时拆分
