...
- **真实模型名**：从 Kimi `config.toml` 的 `display_name` 读取真实模型名（如 "K2.7 Code High Speed"），保持 model key 为 `kimi-for-coding` 以复用定价与图表配色
- **定价**：`pricing_fallback.json` 新增 `kimi-for-coding` 费率
- **UI**：新增 Kimi 标签页、月牙 logo 与 violet 品牌配色（复用 `--kimi` 色板，含 dark/light/glass 变体）

### 说明
- Kimi 为纯使用量集成——`wire.jsonl` 不含 rate-limit 窗口数据，故暂不提供 rate-limit 面板（与 `All` 标签一致）
